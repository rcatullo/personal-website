...
We make the following observation. A complex vector space $V$ is equivalent to a real vector space $V$ along with some $J \in \End_{\bbr}(V)$ such that $J^2 = - \id_V$. Given a complex vector space $V$, we can let $J$ be multiplication by $i$. Given $(V,J)$ as above, we can get a natural $\bbc$-structure on $V$ by letting 
$$
(a+bi)v = av + bJv
$$ 
for all $a,b \in \bbr$ and $v \in V$. 

**Example** Let $M = \bbc^n$, $p \in M$ and $V = T_p M$ with local coordinates $z = (z_1, \ldots, z_n)$. Then each $z_k = x_k + i y_k$ where the $x,y$ are real-valued coordinates, and so $T_p M = \bbr^n \times \bbr^n$ with the local coordinates $\left\{\frac{\del}{\del x_1}, \ldots, \frac{\del}{\del x_n}, \frac{\del}{\del y_1}, \ldots, \frac{\del}{\del y_n}\right\}$. Define
$$
J = \begin{pmatrix}0 & I_n \\ -I_n & 0 \end{pmatrix} \in \End(V)
$$
to be our endomorphism of $V$, and note that $J$ satisfies the following equations.
$$
\begin{align*}
J \frac{\del}{\del x_k} &= \frac{\del}{\del y_k} \\ 
J \frac{\del}{\del y_k} &= -\frac{\del}{\del x_k}
\end{align*}
$$

Note that this extends naturally to families, i.e. to vector bundles, by defining everything fiberwise. That is, a complex vector bundle $\cale$ over a manifold $M$ is a real vector bundle $\cale$ over $M$ together with a $J \in \End(\cale)$ such that for every $p \in M$, the map $J_p \colon \cale_p \to \cale_p$ on the fibers satisfies $J_p^2 = -\id$.

**Remark** Because the notation will get annoying, for $v \in \cale_p$ an element of the fiber over $p \in M$ we write $Jv$ instead of $J_p v$.
...
Since $J$ has two eigenvalues on $V^{\bbc}$, namely $\pm i$, there are two eigenspaces $V^{1,0}$ and $V^{0,1}$ corresponding to $i$ and $-i$, respectively. Then there is a natural decomposition
$$
V^{\bbc} = V^{1,0} \oplus V^{1,0}
$$
such that $V^{1,0} = \bar{V^{0,1}}$, where
$$
V^{1,0} = \{v - i Jv \mid v \in V\} \qquad V^{0,1} = \{v + i J v \mid v \in V\}
$$
Further, there are natural $\bbc$-linear maps $v \mapsto \frac{1}{2}(v \pm i J v)$ that identify $(V,J)$ with $(V^{1,0}, i)$ and $(V^{0,1}, -i)$.
...
where
$$
\cale^{1,0} = \{(p, v - i J v) \mid p \in M, v \in \cale_p\} \qquad \cale^{0,1} = \{(p, v + i J v) \mid p \in M, v \in \cale_p\}
$$
...
$$
Since the notation will get particularly bad otherwise, let us denote the cotangent bundle $T^*M$ by $\Omega M$. We have an induced almost complex structure $J^*$ on $\Omega M$ given by $J^* \eta = -\eta \circ J$ for $\eta \in \Omega_p M = T^*_p M$. Then $(\Omega M, J^*)$ is a complex vector bundle, so we can complexify it as before. Let $\Omega M^{\bbc} = \text{ space of } \bbc \text{-valued functions } = (T^* M)^{\bbc}$. We really want to think of this as the space of **complex differential 1-forms** on $M$. As before,
$$
\Omega M^{\bbc} = \Omega^{1,0} M \oplus \Omega^{0,1}M
$$
Let's unravel the definitions here real quick. Note that by definition,
$$
\begin{align*}
