...
=  \sigma_h'(z^t) \odot \left({h^{t-1}_{\theta}} + U_h\gradient{\left[\frac{\del h^{t-1}_{\theta}}{\del U_h}\right]^{\top}}\right) \\
&= \sigma_h'(z^t) \odot \left({h^{t-1}_{\theta}} + U_h\gradient{\left[\sigma_h'(z^{t-1}) \odot \left({h^{t-2}_{\theta}} + U_h \left[\frac{\del h^{t-2}_{\theta}}{\del U_h} \right]^{\top}\right)\right]}\right) \\
