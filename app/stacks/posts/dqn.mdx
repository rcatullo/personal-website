...
- Compute $Q(s,a;\theta)$ for every action $a$ using the network and pick an action $a$ using the $\eps$-greedy strategy.
...
Recall the $\eps$-greedy strategy, which is a way to balance exploration/exploitation. We choose the optimal action with probability $1-\eps$, and a random action with probability $\eps$. As training progresses, $\eps$ exponentially decays so we learn the optimal policy.
...
- Compute $Q_{\text{online}}(s,a; \theta)$ for every action $a$ using the online network and pick $a$ using the $\eps$-greedy strategy.
...
- Given our current state $s_i$, compute $Q_{\text{online}}(s_i,a; \theta)$ for every action $a$ using the online network and pick $a_i$ using the $\eps$-greedy strategy. Add the transition $(s_i,a_i,r_i,s_{i+1})$ to replay memory buffer $\calm$.
...
Now we implement the $\eps$-greedy strategy, where we decay our epsilon threshold for choosing a random action according to
$$
\eps\text{-threshold} = A + (B - A) e^{-s/D}
$$
