...
Let $\gamma$ be a smooth curve on $M$ as before, and $\sigma$ a section of $\cale$. Then we say $\sigma$ is parallel along $\gamma$ if 
$$
\nabla_{\gamma'(t)} \sigma = 0
$$
for all $t$. Equivalently, consider the pullback connection $\gamma^* \nabla$ on the pullback bundle $\gamma^* \cale$. Then a section $\sigma$ of $\gamma^* \cale$ is parallel if and ony if $\gamma^* \nabla(\sigma) = 0$.